...
router.post('/create', verifyToken, async (req, res) => {
    try {
        ...
        console.error(error);
        return res.status(500).json({ success: false, message: 'Erro na criação da categoria', error });
    }
});
...
router.put('/:id', verifyToken, async (req, res) => {
    try {
        ...
        const existingCategory = await Category.findOne({ where: { id: categoryId, UserId: userId } });
        if (!existingCategory) {
        ...
        console.error(error);
        return res.status(500).json({ success: false, message: 'Erro ao atualizar a categoria', error });
    }
});
...
router.delete('/:id', verifyToken, async (req, res) => {
    try {
        ...
        console.error(error);
        return res.status(500).json({ success: false, message: 'Erro ao excluir categoria', error });
    }
});
